$(document).ready(function()
{
    ...
    let drawn = random(config.number_questions, database.length)
    ...
    function debug()
    {
        $('.debug_display').html('<i style="color:rgb(255, 255, 255);">Correct Answer: </i>' + database[drawn[lvl]][5])
    ...
    function checking_lifebuoy(lifebuoy)
    {
        ...
        {
            sound('50-50')
            if(database[drawn[lvl]][5] == 'A'){$('[answer="C"]').text(''); $('[answer="D"]').text('')}
            if(database[drawn[lvl]][5] == 'B'){$('[answer="A"]').text(''); $('[answer="C"]').text('')}
            if(database[drawn[lvl]][5] == 'C'){$('[answer="D"]').text(''); $('[answer="B"]').text('')}
            if(database[drawn[lvl]][5] == 'D'){$('[answer="A"]').text(''); $('[answer="B"]').text('')}
        }
        if(lifebuoy == 2)
        {
            sound('phone')
            ...
            {
                sound('phone-end')
                alert_game('A friend suggests: '+database[drawn[lvl]][5])
                clearTimeout(ss)
            }, 5000)
        }
        if(lifebuoy == 3){
            ...
            {
                sound('phone-end')
                alert_game('The audience voted for: '+database[drawn[lvl]][5])
                clearTimeout(ss)
            }, 5000)
        }
    }
    ...
    {
        if(database[drawn[lvl]][5] === answer)
        {
            lvl++
            ...
            $(this).animate({opacity: 1}, 1000*config.duration, function()
            {
                ...
                {
                    $('.question').text(database[drawn[lvl]][0])
                    $('[answer="A"]').text(database[drawn[lvl]][1])
                    $('[answer="B"]').text(database[drawn[lvl]][2])
                    $('[answer="C"]').text(database[drawn[lvl]][3])
                    $('[answer="D"]').text(database[drawn[lvl]][4])
                }
            })
        }
        else
        {
            ...
            $('[answer="'+database[drawn[lvl]].correct_answer+'"]').css({'backgroundColor':config.green, 'opacity':1})
            $(this).animate({opacity: 1}, 1000*config.duration, function()
            {
                $('[answer]').attr('blc', 0).css({'backgroundColor':config.backgroundColor, 'opacity':1})
                game_over( lvl-1 )
    ...
    function game_start()
    {
        ...
        lvl = 1
        drawn = random(config.number_questions, database.length)
